  ...
  return (
    <div className="space-y-6">
      ...
      <article className="rounded-xl border border-border bg-surface p-5">
        ...
        <p className="mt-0.5 text-xs text-ink-soft">
          Format imposé : AAAA-AAAA. La clôture d'une année se fait dans{' '}
          <Link to="/year-closing" className="font-medium text-primary">
            Recouvrement › Clôture d'année
          </Link>
          .
        </p>
          ...
          {academicYears?.map((year) => (
            <div key={year.id} className="flex items-center justify-between px-4 py-2.5 text-sm">
              ...
              <div className="flex items-center gap-2">
                ...
                {!year.closed_at && !year.is_active && canManageYears && (
                  <button
                    type="button"
