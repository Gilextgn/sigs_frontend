...
import { useCreateSubject, useDeleteSubject, useSubjects, type SubjectRow } from './useTeaching';
...
export default function SubjectsPage() {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    setError(null);
    try {
      await createSubject.mutateAsync({ code: code.trim().toUpperCase(), label: label.trim() });
      setCode('');
      setLabel('');
      setModalOpen(false);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
        ...
        <button type="button" onClick={() => setModalOpen(true)} className="flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-white transition hover:bg-primary-dark">
        ...
        <Modal title="Nouvelle matière" onClose={() => setModalOpen(false)}>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-end gap-2">
              <button type="button" onClick={() => setModalOpen(false)} className="rounded-lg border border-border px-4 py-2 text-sm font-medium text-ink">Annuler</button>
              <button type="submit" disabled={createSubject.isPending} className="rounded-lg bg-primary px-4 py-2 text-sm font-medium text-white disabled:opacity-60">{createSubject.isPending ? 'Enregistrement...' : 'Enregistrer'}</button>
            </div>
          </form>
        </Modal>
      )}
